...
internal class JsonSchemaExtractorTest {
    ...
    fun `init test project`() {
        ...
        extractor = JsonSchemaExtractor(File(testProject, "charts"), extractSchemasDir)
    }
    ...
    fun `extract should extract JSON schemas from dependency archives`() {
        val chart = Chart("v2", CHART_NAME, CHART_VERSION, listOf(ChartDependency(EMBEDDED_SCHEMA, SUBCHART_VERSION, THIRDPARTY)))
    ...
    fun `extract should extract sub-charts JSON schemas from dependency archives`() {
        val chart = Chart("v2", CHART_NAME, CHART_VERSION, listOf(ChartDependency(EMBEDDED_SUB_SCHEMA, SUBCHART_VERSION, THIRDPARTY)))
    ...
    fun `extract should generate JSON schema with error when archive is not found for dependency`() {
        val chart = Chart("v2", CHART_NAME, CHART_VERSION, listOf(ChartDependency(MISSING_ARCHIVE, SUBCHART_VERSION, THIRDPARTY)))
        extractor.extract(chart)
    ...
    fun `extract should generate JSON schema with error when archive is invalid`() {
        val chart = Chart("v2", CHART_NAME, CHART_VERSION, listOf(ChartDependency(INVALID_ARCHIVE, SUBCHART_VERSION, THIRDPARTY)))
    ...
    fun `extract should create empty extract directory when dependencies contain no JSON schema`() {
        val chart = Chart("v2", CHART_NAME, CHART_VERSION, listOf(ChartDependency(NO_SCHEMA, SUBCHART_VERSION, THIRDPARTY)))
